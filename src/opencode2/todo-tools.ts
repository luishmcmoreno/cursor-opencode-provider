 ...
 */
export const CURSOR_OPENCODE2_TODOS_ENV = "CURSOR_OPENCODE2_TODOS"

export function isOpenCode2TodosEnabled(
  env: NodeJS.ProcessEnv = process.env,
): boolean {
  const value = env[CURSOR_OPENCODE2_TODOS_ENV]?.toLowerCase()
...
export const TODOREAD_DESCRIPTION =
  "Read the current session todo list. Takes no arguments. Returns the full list as JSON."
...
export function registerTodoTools(draft: ToolDraft): void {
  ...
  if (!hostHasTool(draft, TODOWRITE_TOOL)) {
    draft.add({
      ...
      execute: async (input: { todos?: unknown }, context: unknown) => {
        const sessionID = sessionIDFromContext(context)
        const todos = setSessionTodos(sessionID, input?.todos)
